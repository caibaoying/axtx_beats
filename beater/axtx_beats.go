...
var (
	once = flag.Bool("once", false, "Run axtx_beats only once until all harvesters reach EOF")
)

// Axtx_beats is a beater object. Contains all objects needed to run the beat
...
// New creates a new Axtx_beats pointer instance.
func New(b *beat.Beat, rawConfig *common.Config) (beat.Beater, error) {
	...
	// config.Prospectors can't append
...
func (fb *Axtx_beats) modulesSetup(b *beat.Beat) error {
	...
	if esConfig == nil || !esConfig.Enabled() {
		logp.Warn("axtx_beats is unable to load the Ingest Node pipelines for the configured" +
...
func (fb *Axtx_beats) Run(b *beat.Beat) error {
	var err error
	config := fb.config
	...
	//registry是registrar创建之后传递给crawler的，里面是文件的处理状态记录。
	...
	if *once {
		runOnce := func() {
			logp.Info("Running axtx_beats once. Waiting for completion ...")
...
func (fb *Axtx_beats) Stop() {
	logp.Info("Stopping axtx_beats")
